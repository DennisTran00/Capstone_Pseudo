...
public class DBConnection {
	...
	public ArrayList<User> GetUsers() {
		...
		try {
			...
			while (result.next()) {
				User u = new User(result.getString("username"), result.getString("password"), result.getString("ID"));
				users.add(u);
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public void AddUser(User user) {
		try {
			...
			insert.executeUpdate("INSERT INTO user_info VALUES (\"" + user.getUsername() + "\",\"" + user.getPassword() + "\",\"" + user.getUserID() + "\");");
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
	...
	public void AddCredential(Credential credential) {
		try {
			Statement insert = dbCon.createStatement();
			insert.executeUpdate("INSERT INTO credentials VALUES (\"" + credential.getUserID() + "\",\"" + credential.getUsername() + "\",\"" + credential.getPassword() +
	...
	public ArrayList<Credential> GetCredentials(User user) {
		...
		try {
			...
			while (result.next()) {
				Credential c = new Credential(result.getString("Username"), result.getString("Password"), result.getString("UserID"), result.getString("CreationDate"), result.getString("ModifiedDate"), result.getString("Notes"), result.getString("URL"), result.getInt("Created"), result.getString("Title"));
				credentials.add(c);
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public void UpdateCredential(Credential curr, Credential cred) {
		try {
			...
			update.executeUpdate("UPDATE credentials SET Title = \"" + cred.getTitle() + "\", Username = \"" + cred.getUsername() + "\", Password = \"" + cred.getPassword() + "\", URL = \"" + cred.getURL() + "\", ModifiedDate = \"" + cred.getModifiedTime() + "\" WHERE UserID = \"" + curr.getUserID() + "\" AND Title = \"" + curr.getTitle() + "\";");
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
	...
	public void DeleteCredential(Credential cred) {
		try {
			...
			delete.executeUpdate("DELETE FROM credentials WHERE UserID = \"" + cred.getUserID() + "\" AND Username = \"" + cred.getUsername() + "\" AND Password = \"" + cred.getPassword() + "\";");
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
		...
		try {
			...
			if (result.next()) {
				...
				while (result2.next()) {
					...
					c.setUsername(result2.getString("Username"));
					c.setPassword(result2.getString("Password"));
					r.addPrivilege(c);
				}
	...
	public ArrayList<String> getRolesFromRoleName(Team team, String roleName) {
		...
		try {
			...
			while (result.next()) {
				rn.add(result.getString("Username"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public void AddRole(Credential cred, Team team, Role role) {
		try {
			Statement insert = dbCon.createStatement();
			insert.executeUpdate("INSERT INTO roles VALUES (\"" + team.getTeamID() + "\",\"" + role.getRoleID() + "\",\"" + role.getRoleName() + "\",\"" + cred.getUsername() + "\",\"" + cred.getPassword() + "\");");
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
